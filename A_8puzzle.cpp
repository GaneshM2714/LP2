...
// int get_manhattan_h(const vector<vector<int>>& src, const vector<vector<int>>& dest) {
//     unordered_map<int, pair<int, int>> goal_pos;
//     for (int i = 0; i < dest.size(); ++i) {
//         for (int j = 0; j < dest[0].size(); ++j) {
//             goal_pos[dest[i][j]] = {i, j};
//         }
//     }
...
//             int val = src[i][j];
//             if (val != 0) {
//                 auto [goal_i, goal_j] = goal_pos[val];
...
// bool is_solvable(vector<vector<int>> board) {
//     vector<int> flat;
//     for (auto& row : board)
//         for (int num : row)
//             if (num != 0)
